...
from tests.helpers import (
    wait_for_public_ip_pool_api_ready,
...
def test_public_ip_pool_lifecycle(grpc: GRPCClient, k8s_hub_client: K8sClient) -> None:
    ...
    pool_cr: str = wait_for_public_ip_pool_cr(k8s=k8s_hub_client, uuid=pool_id)
    wait_for_public_ip_pool_ready(k8s=k8s_hub_client, name=pool_cr)
    wait_for_public_ip_pool_api_ready(grpc=grpc, pool_id=pool_id)

    assert pool_id in grpc.list_public_ip_pool_ids()

    pool = grpc.get_public_ip_pool(pool_id=pool_id)
    assert pool["object"]["spec"]["cidrs"] == ["198.51.100.0/28"]

    grpc.delete_public_ip_pool(pool_id=pool_id)
    wait_for_public_ip_pool_deletion(k8s=k8s_hub_client, name=pool_cr)
    poll_until(
